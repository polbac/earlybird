...
class AuthorizationService {
    ...
    async processAST(ast, action = 'read') {
        ...
        function validateFields(ast) {
            ...
            function checkChildren(collection, children) {
        ...
        function applyFilters(ast, accountability) {
            ...
            function updateFilterQuery(collection, query) {
                // We check the availability of the permissions in the step before this is run
                const permissions = permissionsForCollections.find((permission) => permission.collection === collection);
                const parsedPermissions = utils_1.parseFilter(permissions.permissions, accountability);
    ...
    validatePayload(action, collection, data) {
        ...
        const preset = utils_1.parseFilter(permission.presets || {}, this.accountability);
        const payloadWithPresets = lodash_1.merge({}, preset, payload);
        const hasValidationRules = lodash_1.isNil(permission.validation) === false && Object.keys((_c = permission.validation) !== null && _c !== void 0 ? _c : {}).length > 0;
        ...
        validationErrors.push(...lodash_1.flatten(utils_1.validatePayload(utils_1.parseFilter(permission.validation, this.accountability), payloadWithPresets).map((error) => error.details.map((details) => new exceptions_2.FailedValidationException(details)))));
