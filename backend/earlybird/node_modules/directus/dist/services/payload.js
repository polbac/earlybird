...
class PayloadService {
    ...
    async processValues(action, payload) {
        const processedPayload = utils_1.toArray(payload);
    ...
    async processField(field, payload, action, accountability) {
        ...
        const fieldSpecials = field.special ? utils_1.toArray(field.special) : [];
    ...
    processGeometries(payloads, action) {
        ...
        const process = action == 'read'
            ? (value) => {
                if (typeof value === 'string')
                    return wellknown_1.parse(value);
            }
            : (value) => helper.fromGeoJSON(typeof value == 'string' ? JSON.parse(value) : value);
        const fieldsInCollection = Object.entries(this.schema.collections[this.collection].fields);
        const geometryColumns = fieldsInCollection.filter(([_, field]) => geometry_1.isNativeGeometry(field));
            ...
            for (const payload of payloads) {
                ...
                if (action === 'read') {
                    if (value instanceof Date)
                        payload[name] = date_fns_1.format(value, 'HH:mm:ss');
                }
            }
        }
        return payloads;
    ...
    async processA2O(data) {
        ...
        const relationsToProcess = relations.filter((relation) => {
            return relation.field in payload && lodash_1.isPlainObject(payload[relation.field]);
        });
        for (const relation of relationsToProcess) {
            ...
            const exists = hasPrimaryKey &&
                !!(await this.knex
                    ...
                    .first());
            if (exists) {
                const fieldsToUpdate = lodash_1.omit(relatedRecord, relatedPrimary);
    ...
    async processM2O(data) {
        ...
        // Only process related records that are actually in the payload
        const relationsToProcess = relations.filter((relation) => {
            return relation.field in payload && lodash_1.isObject(payload[relation.field]);
        });
        for (const relation of relationsToProcess) {
            ...
            const exists = hasPrimaryKey &&
                !!(await this.knex
                    ...
                    .first());
            if (exists) {
                const fieldsToUpdate = lodash_1.omit(relatedRecord, relatedPrimaryKeyField);
                if (Object.keys(fieldsToUpdate).length > 0) {
                    await itemsService.updateOne(relatedPrimaryKey, relatedRecord, {
                        onRevisionCreate: (id) => revisions.push(id),
                    });
            ...
            if (Array.isArray(payload[relation.meta.one_field])) {
                for (let i = 0; i < (payload[relation.meta.one_field] || []).length; i++) {
                    ...
                    if (typeof relatedRecord === 'string' || typeof relatedRecord === 'number') {
                        ...
                        if (lodash_1.isNil(existingRecord[relation.field]) === false &&
