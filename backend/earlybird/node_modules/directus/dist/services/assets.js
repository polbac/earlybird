...
const semaphore = new async_mutex_1.Semaphore(env_1.default.ASSETS_MAX_CONCURRENT_TRANSFORMATIONS);
class AssetsService {
    constructor(options) {
    ...
    async getAsset(id, transformation, range) {
        ...
        if (type && transforms.length > 0 && ['image/jpeg', 'image/png', 'image/webp', 'image/tiff'].includes(type)) {
            ...
            if (maybeNewFormat) {
                file.type = mime_types_1.contentType(assetFilename) || null;
            }
            if (exists) {
                return {
            ...
            return await semaphore.runExclusive(async () => {
                ...
                const transformer = sharp_1.default({
...
const getAssetSuffix = (transforms) => {
    if (Object.keys(transforms).length === 0)
        return '';
    return `__${object_hash_1.default.sha1(transforms)}`;
};
