...
exports.validateMigrations = exports.isInstalled = exports.validateDBConnection = exports.hasDatabaseConnection = exports.getSchemaInspector = void 0;
...
function getDatabase() {
    ...
    validate_env_1.validateEnv(requiredEnvVars);
    const knexConfig = {
    ...
    if (env_1.default.DB_CLIENT === 'mssql') {
        ...
        // act the same
        lodash_1.merge(knexConfig, { connection: { options: { useUTC: false } } });
    }
    database = knex_1.knex(knexConfig);
    const times = {};
    database
        .on('query', (queryInfo) => {
...
function getSchemaInspector() {
    ...
    inspector = schema_1.default(database);
    return inspector;
}
...
async function validateDBConnection(database) {
    database = database !== null && database !== void 0 ? database : getDatabase();
    try {
        if (env_1.default.DB_CLIENT === 'oracledb') {
            await database.raw('select 1 from DUAL');
