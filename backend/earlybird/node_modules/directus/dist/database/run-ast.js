...
async function runAST(originalAST, schema, options) {
    ...
    async function run(collection, children, query) {
        ...
        const { columnsToSelect, primaryKeyField, nestedCollectionNodes } = await parseCurrentLevel(schema, collection, children);
        // The actual knex query builder instance. This is a promise that resolves with the raw items from the db
        const dbQuery = await getDBQuery(schema, knex, collection, columnsToSelect, query, options === null || options === void 0 ? void 0 : options.nested);
...
async function parseCurrentLevel(schema, collection, children) {
...
function getColumnPreprocessor(knex, schema, table) {
    const helper = geometry_2.getGeometryHelper();
    return function (column) {
        const field = schema.collections[table].fields[column];
        if (geometry_1.isNativeGeometry(field)) {
            return helper.asText(table, column);
        }
        return knex.raw('??.??', [table, column]);
    };
}
function getDBQuery(schema, knex, table, columns, query, nested) {
...
function applyParentFilters(schema, nestedCollectionNodes, parentItem) {
    var _a;
    const parentItems = utils_1.toArray(parentItem);
            ...
            nestedNode.query = {
                ...
                filter: {
                    ...
                    [schema.collections[nestedNode.relation.related_collection].primary]: {
                        _in: lodash_1.uniq(parentItems.map((res) => res[nestedNode.relation.field])).filter((id) => id),
                    },
                },
            };
        }
...
function mergeWithParentItems(schema, nestedItem, parentItem, nestedNode, nested) {
    var _a, _b;
    const nestedItems = utils_1.toArray(nestedItem);
    const parentItems = lodash_1.clone(utils_1.toArray(parentItem));
    if (nestedNode.type === 'm2o') {
        for (const parentItem of parentItems) {
...
function removeTemporaryFields(schema, rawItem, ast, primaryKeyField, parentItem) {
    const rawItems = lodash_1.cloneDeep(utils_1.toArray(rawItem));
    const items = [];
    if (ast.type === 'm2a') {
        const fields = {};
        const nestedCollectionNodes = {};
