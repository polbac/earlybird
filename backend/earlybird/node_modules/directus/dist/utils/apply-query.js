...
function applyFilter(schema, rootQuery, rootFilter, collection, subQuery = false) {
    ...
    function addJoins(dbQuery, filter, collection) {
        ...
        function addJoin(path, collection) {
            ...
            function followRelation(pathParts, parentCollection = collection, parentAlias) {
                ...
                if (!relation)
                    return;
                const relationType = get_relation_type_1.getRelationType({ relation, collection: parentCollection, field: pathRoot });
    ...
    function addWhereClauses(dbQuery, filter, collection, logical = 'and') {
        for (const [key, value] of Object.entries(filter)) {
            ...
            const relationType = relation ? get_relation_type_1.getRelationType({ relation, collection: collection, field: pathRoot }) : null;
            if (relationType === 'm2o' || relationType === 'm2a' || relationType === null) {
                ...
                dbQuery[logical].whereIn(pkField, (subQueryKnex) => {
                    ...
                    applyQuery(relation.collection, subQueryKnex, {
                        filter: value,
                    }, schema, true);
                });
            }
        ...
        function applyFilterToQuery(key, operator, compareValue, logical = 'and') {
            ...
            const geometryHelper = geometry_1.getGeometryHelper();
