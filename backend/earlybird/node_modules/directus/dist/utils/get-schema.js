...
async function getSchema(options) {
    const database = (options === null || options === void 0 ? void 0 : options.database) || database_1.default();
    const schemaInspector = schema_1.default(database);
    const { schemaCache } = cache_1.getCache();
    let result;
    if (env_1.default.CACHE_SCHEMA !== false && schemaCache) {
        ...
        if (cachedSchema) {
            ...
            try {
                await schemaCache.set('schema', result, typeof env_1.default.CACHE_SCHEMA === 'string' ? ms_1.default(env_1.default.CACHE_SCHEMA) : undefined);
            }
            catch (err) {
...
async function getDatabaseSchema(database, schemaInspector) {
        ...
        result.collections[collection] = {
            ...
            fields: lodash_1.mapValues(schemaOverview[collection].columns, (column) => {
                ...
                return ({
                    ...
                    type: column ? get_local_type_1.default(column).type : 'alias',
    ...
    ].filter((field) => (field.special ? utils_1.toArray(field.special) : []).includes('no-data') === false);
    for (const field of fields) {
        if (!result.collections[field.collection])
            continue;
        const existing = result.collections[field.collection].fields[field.field];
        const column = schemaOverview[field.collection].columns[field.field];
        const special = field.special ? utils_1.toArray(field.special) : [];
        const { type = 'alias' } = existing && column ? get_local_type_1.default(column, { special }) : {};
