...
function getJoi(operator, value) {
    ...
    if (operator === '_starts_with') {
        return Joi.string().pattern(new RegExp(`^${lodash_1.escapeRegExp(value)}.*`), { name: 'starts_with' });
    }
    if (operator === '_nstarts_with') {
        return Joi.string().pattern(new RegExp(`^${lodash_1.escapeRegExp(value)}.*`), { name: 'starts_with', invert: true });
    }
    if (operator === '_ends_with') {
        return Joi.string().pattern(new RegExp(`.*${lodash_1.escapeRegExp(value)}$`), { name: 'ends_with' });
    }
    if (operator === '_nends_with') {
        return Joi.string().pattern(new RegExp(`.*${lodash_1.escapeRegExp(value)}$`), { name: 'ends_with', invert: true });
    }
    if (operator === '_in') {
        return Joi.any().equal(...value);
    }
