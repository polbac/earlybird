...
function extractOptions(keys, numberKeys = []) {
    return function (input) {
        return Object.entries(input).reduce((config, [key, value]) => keys.includes(key) && lodash_1.isNil(value) === false
            ? {
                ...config,
                [key]: numberKeys.includes(key) ? +value : value,
            }
            : config, {});
    };
}
...
function extractResize(input) {
    ...
    return [
        'resize',
        extractOptions(['width', 'height', 'fit', 'withoutEnlargement'], ['width', 'height'])(input),
    ];
}
/**
