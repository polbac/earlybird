...
exports.respond = async_handler_1.default(async (req, res) => {
    var _a, _b, _c;
    const { cache } = cache_1.getCache();
    if (req.method.toLowerCase() === 'get' &&
        ...
        try {
            await cache.set(key, res.locals.payload, ms_1.default(env_1.default.CACHE_TTL));
            await cache.set(`${key}__expires_at`, Date.now() + ms_1.default(env_1.default.CACHE_TTL));
        }
        catch (err) {
            logger_1.default.warn(err, `[cache] Couldn't set key ${key}. ${err}`);
        }
        res.setHeader('Cache-Control', get_cache_headers_1.getCacheControlHeader(req, ms_1.default(env_1.default.CACHE_TTL)));
    ...
    if (req.sanitizedQuery.export) {
        ...
        if (req.sanitizedQuery.export === 'xml') {
            ...
            return res.status(200).send(js2xmlparser_1.parse('data', (_b = res.locals.payload) === null || _b === void 0 ? void 0 : _b.data));
        }
