...
const setup = () => express_1.default().use(cache_1.default).all('/items/test', handler);
...
describe('cache middleware', () => {
    test('should return the cached response for a request', async () => {
        ...
        const res = await supertest_1.default(setup()).get('/items/test').send();
    ...
    test('should not use cache when the "Cache-Control" header is set to "no-store"', async () => {
        const res = await supertest_1.default(setup()).get('/items/test').set('Cache-Control', 'no-store').send();
        expect(res.body.data).toBe('Uncached value');
        expect(cache.get).not.toHaveBeenCalled();
        expect(handler).toHaveBeenCalledTimes(1);
    });
    test('should only cache get requests', async () => {
        const app = setup();
        await supertest_1.default(app).post('/items/test').send();
        await supertest_1.default(app).put('/items/test').send();
        await supertest_1.default(app).patch('/items/test').send();
        await supertest_1.default(app).delete('/items/test').send();
        expect(cache.get).not.toHaveBeenCalled();
