...
function validatePayload(filter, payload, options) {
    ...
    if (Object.keys(filter)[0] === '_and') {
        ...
        const subValidation = Object.values(filter)[0];
        const nestedErrors = lodash_1.flatten(subValidation.map((subObj) => validatePayload(subObj, payload, options)));
        ...
        const schema = generate_joi_1.generateJoi(filter, options);
